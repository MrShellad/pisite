...
pub async fn update_settings(
    ...
    sqlx::query("UPDATE site_settings SET site_name=?, site_domain=?, seo_title=?, seo_description=?, seo_keywords=?, github_url=?, twitter_url=?, discord_url=?, contact_email=?, copyright=? WHERE id='1'")
    .bind(payload.site_name).bind(payload.site_domain.trim().trim_end_matches('/')).bind(payload.seo_title).bind(payload.seo_description).bind(payload.seo_keywords)

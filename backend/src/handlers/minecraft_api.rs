...
pub async fn get_latest_update(State(pool): State<SqlitePool>) -> Result<Json<McUpdate>, (StatusCode, String)> {
    // 1. 客户端请求次数 +1
    sqlx::query("UPDATE mc_crawler_config SET request_count = request_count + 1 WHERE id = '1'")
        .execute(&pool).await.ok();

    // 2. 直接从数据库拉取最新的一条 (按日期或创建时间降序)
    let latest = sqlx::query_as::<_, McUpdate>("SELECT * FROM mc_updates ORDER BY date DESC, created_at DESC LIMIT 1")
        .fetch_optional(&pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    match latest {
        Some(update) => Ok(Json(update)),
        None => Err((StatusCode::NOT_FOUND, "缓存为空，爬虫可能还未完成首次抓取".to_string()))
    }
}
...
pub async fn get_crawler_config(_claims: Claims, State(pool): State<SqlitePool>) -> Json<McCrawlerConfig> {
    let config = sqlx::query_as::<_, McCrawlerConfig>("SELECT * FROM mc_crawler_config WHERE id = '1'")
...
pub async fn update_crawler_config(
    ...
    sqlx::query("UPDATE mc_crawler_config SET interval_minutes = ? WHERE id = '1'")
        .bind(safe_interval).execute(&pool).await.unwrap();
    Ok(StatusCode::OK)
}

// 获取缓存的历史卡片列表
pub async fn get_cached_updates(_claims: Claims, State(pool): State<SqlitePool>) -> Json<Vec<McUpdate>> {
...
// 手动强制触发一次抓取
pub async fn force_crawl(_claims: Claims, State(pool): State<SqlitePool>) -> Result<StatusCode, (StatusCode, String)> {
...
pub async fn crawler_daemon(pool: SqlitePool) {
    loop {
        ...
        let config: Result<McCrawlerConfig, _> = sqlx::query_as("SELECT * FROM mc_crawler_config WHERE id = '1'")
...
#[derive(Deserialize)]
struct MojangManifest { versions: Vec<VersionInfo> }
#[derive(Deserialize)]
struct VersionInfo { id: String, #[serde(rename = "type")] v_type: String }
#[derive(Deserialize)]
struct JsonLdSchema { headline: Option<String>, image: Option<Vec<String>>, #[serde(rename = "datePublished")] date_published: Option<String> }

async fn perform_crawl(pool: &SqlitePool) -> Result<(), String> {
    // 1. 获取 Mojang 最新清单
    let manifest: MojangManifest = reqwest::get("https://piston-meta.mojang.com/mc/game/version_manifest_v2.json")
        .await.map_err(|e| format!("请求清单失败: {}", e))?
        .json().await.map_err(|e| format!("解析清单失败: {}", e))?;
    ...
    let exists: (bool,) = sqlx::query_as("SELECT EXISTS(SELECT 1 FROM mc_updates WHERE version = ?)")
        .bind(v_id).fetch_one(pool).await.unwrap_or((false,));

    if !exists.0 {
        // 执行重度网页抓取
        let (article_url, wiki_en, wiki_zh) = build_urls(v_id, v_type);
        let html = reqwest::get(&article_url).await.map_err(|e| e.to_string())?.text().await.unwrap_or_default();
        ...
        let schema_data = {
            ...
            for el in document.select(&selector) {
                ...
                if inner.contains("NewsArticle") {
                    if let Ok(parsed) = serde_json::from_str::<JsonLdSchema>(&inner) { extracted = Some(parsed); break; }
                }
            }
            extracted
        };
        ...
        if let Some(schema) = schema_data {
            let title = format!("{} 更新日志", schema.headline.unwrap_or(format!("Minecraft {}", v_id)));
            let date = schema.date_published.unwrap_or_default().split('T').next().unwrap_or("").to_string();
            let source_image = schema.image.and_then(|imgs| imgs.into_iter().next()).unwrap_or_default();
...
fn build_urls(version_id: &str, v_type: &str) -> (String, String, String) {
    ...
    if v_type == "release" && !version_id.contains('-') {
        ...
        return (format!("{}/{}", base_url, article_id), format!("{}_{}", wiki_en_base, version_id), format!("{}{}", wiki_zh_base, version_id));
    }
    if version_id.contains('w') && !version_id.contains('-') {
        return (format!("{}/minecraft-snapshot-{}", base_url, version_id), format!("{}_{}", wiki_en_base, version_id), format!("{}{}", wiki_zh_base, version_id));
    }
    ...
    let (article_suffix, en_wiki_suffix, zh_wiki_suffix) = if suffix.starts_with("snapshot") {
        ...
        (format!("snapshot-{}", num), format!("Snapshot_{}", num), format!("snapshot-{}", num))
    } else if suffix.starts_with("pre") {
        let num = suffix.replace("pre", "").replace("-", ""); 
        (format!("pre-release-{}", num), format!("Pre-Release_{}", num), format!("pre{}", num))
    } else if suffix.starts_with("rc") {
        let num = suffix.replace("rc", "").replace("-", "");
        (format!("release-candidate-{}", num), format!("Release_Candidate_{}", num), format!("rc{}", num))
    } else {
        (suffix.to_string(), suffix.to_string(), suffix.to_string())
    };

    (
        format!("{}/minecraft-{}-{}", base_url, base_version.replace('.', "-"), article_suffix),

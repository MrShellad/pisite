...
pub async fn get_faqs(State(pool): State<SqlitePool>) -> Json<Vec<Faq>> {
    let faqs = sqlx::query_as::<_, Faq>("SELECT * FROM faqs WHERE enabled = 1 ORDER BY priority ASC")
...
// 切换 FAQ 启用状态
pub async fn toggle_faq(_claims: Claims, State(pool): State<SqlitePool>, Path(id): Path<String>) -> Result<StatusCode, (StatusCode, String)> {
...
// 删除 FAQ
pub async fn delete_faq(_claims: Claims, State(pool): State<SqlitePool>, Path(id): Path<String>) -> Result<StatusCode, (StatusCode, String)> {

...
function getCurrentPageName(pathname: string) {
  const nav = [...primaryNavs, ...settingsNavs].find(item => isPathActive(pathname, item.path));
        ...
        <div className="mt-5">
          ...
          >
            <span className="inline-flex min-w-0 items-center gap-3">
              <Settings2 size={18} />
              <span className="truncate">设置中心</span>
            </span>
            <ChevronDown
              size={16}
              className={`shrink-0 transition-transform duration-300 ${shouldShowSettings ? 'rotate-180' : ''}`}
            />
          </button>

          <div
            className={`grid overflow-hidden transition-all duration-300 ${
              shouldShowSettings ? 'mt-1.5 grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-85'
            }`}
          >
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="relative flex h-full w-[min(22rem,calc(100vw-2rem))] flex-col overflow-hidden border-r border-neutral-200 bg-white shadow-2xl dark:border-white/10 dark:bg-neutral-950">
            ...
            <button
              type="button"
              onClick={() => setIsMobileNavOpen(false)}
              className="absolute right-4 top-5 inline-flex h-10 w-10 items-center justify-center rounded-xl border border-neutral-200 bg-white text-neutral-700 shadow-sm transition hover:border-orange-300 hover:text-orange-600 dark:border-white/10 dark:bg-white/5 dark:text-neutral-200"
              aria-label="关闭导航"
            >
              <X size={18} />
            </button>
        ...
        <header className="sticky top-0 z-30 flex min-h-16 shrink-0 items-center justify-between gap-3 border-b border-neutral-200/80 bg-neutral-50/90 px-4 backdrop-blur-xl dark:border-white/10 dark:bg-neutral-950/80 sm:px-6 lg:min-h-20 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <button
              ...
              className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-neutral-200 bg-white text-neutral-700 shadow-sm transition hover:border-orange-300 hover:text-orange-600 dark:border-white/10 dark:bg-white/5 dark:text-neutral-200 lg:hidden"
              aria-label="打开导航"
            >
              <Menu size={18} />
            </button>
            <div className="min-w-0">
              <h2 className="truncate text-lg font-bold text-neutral-900 dark:text-white/90 sm:text-xl">
                {currentPageName}
              </h2>
              <p className="hidden text-xs font-medium text-neutral-500 dark:text-neutral-500 sm:block">
                管理中心
              </p>
            </div>
          </div>
